...
from networks_pay import AttnVGG_before
...
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=BATCH_SIZE, sampler=sampler, num_workers=16)
              for x in ['train']}                                             

num_classes = int(sys.argv[5]) # no of classes
no_of_epochs = int(sys.argv[6]) # no of epochs to train

net = AttnVGG_before(im_size=224, num_classes=num_classes,
            attention=True, normalize_attn=True, init='xavierUniform')
#net = AttnVGG_before(num_classes=num_classes, attention=True, normalize_attn=True)
